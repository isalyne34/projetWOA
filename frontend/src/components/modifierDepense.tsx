...
type ModifierDepenseProps = {
   depense: Depense;
};
export default function ModifierDepense({ depense }: ModifierDepenseProps) {
   function modifierDepense(values: any) {
      fetch(API_URL + `/depensesud/${depense.id_depense}`, {
         method: 'PUT',
         headers: {
   ...
   return (
      <>
         <button type="button" className="btn btn-warning" data-bs-toggle="modal" data-bs-target={`#modifierDepense${depense.id_depense}`}>
         ...
         <div className="modal" tabIndex={-1} id={`modifierDepense${depense.id_depense}`}>
