...
const updateDepense = (req, res) => {
   const id = parseInt(req.params.id);
   const { titre, description, montant, date_crea, id_utilisateur } = req.body;
   db.query(
      'UPDATE depense SET titre=$1,description=$2,montant=$3,date_crea=$4,id_utilisateur=$5 WHERE id_depense=$6',
      [titre, description, montant, date_crea, id_utilisateur, id],
      (error, results) => {
         if (error) {
            throw error;
         }
